...
export default function SocialShare({ title, url }: Props) {
  ...
  return (
    <div className="muted" aria-label="Share">
      <a href={`https://twitter.com/intent/tweet?text=${text}&url=${shareUrl}`} target="_blank" rel="noreferrer">Twitter</a>
      {' · '}
      <a href={`https://www.linkedin.com/sharing/share-offsite/?url=${shareUrl}`} target="_blank" rel="noreferrer">LinkedIn</a>
      {' · '}
      <a href={`mailto:?subject=${text}&body=${shareUrl}`}>Email</a>
    </div>
  )
}
